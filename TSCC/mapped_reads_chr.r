...
ggsave(paste0(out_path, "/mapped_chrs_lib_reads_box.png"))

ggplot(metadata2) +
    geom_boxplot(aes(x=library_name, y=value, color=chr))+
    ylab("# of mapped reads (million)")
ggsave(paste0(out_path, "/mapped_lib_chrs_reads_box.png"))
...
ggsave(paste0(result_dir, "/fraction_mapeed_chrs_lib_ratio_box.png"))

ggplot(metadata2) +
    geom_boxplot(aes(x=library_name, y=value, color=chr))+
    ylab("ratio of mapped reads")
ggsave(paste0(result_dir, "/fraction_mapeed_lib_chrs_ratio_box.png"))
...
ggsave(paste0(out_path, "/fraction_mapeed_on_chrY_chrX_scatter.png"))
...
ggsave(paste0(out_path, "/fraction_mapeed_on_chrX_sex_filter_scatter.png"))

ggplot(metadata,aes(x=index,y=metadata[,"chrY_fra"],shape=library_name,color=sex)) +
    geom_point(size=2) +
    xlab("Sample") +
    ylab("Ratio of mapped reads on chrY") #+ theme(legend.position="none")
ggsave(paste0(out_path, "/fraction_mapeed_on_chrY_sex_filter_scatter.png"))
